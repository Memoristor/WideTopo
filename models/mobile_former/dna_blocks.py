...
class Local2Global(nn.Module):
    ...
    def forward(self, x):
        ...
        if "attn" in self.block:
            ...
            )  # from T x bs x Ct to bs x N x T x Ct/N
...
class GlobalBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if "attn" in self.block:
            ...
            k = tokens.permute(1, 2, 0).view(
                bs, self.num_heads, -1, T
            )  # from T x bs x Ct -> bs x Ct x T -> bs x N x Ct/N x T
    ...
    def forward(self, x):
        ...
        if "attn" in self.block:
            if self.remove_proj_local:
                ...
                q = (
                    self.q(out).view(bs, self.num_heads, -1, H * W).transpose(-1, -2)
                )  # bs x N x HW x C/N

            k = (
                self.k(tokens).permute(1, 2, 0).view(bs, self.num_heads, -1, self.token_num)
            )  # from T x bs x Ct -> bs x C x T -> bs x N x C/N x T
...
class MergeClassifier(nn.Module):
    def __init__(
        ...
        self.fc = nn.Sequential(
            nn.Linear(hidden_dim + cat_token_dim, oup), nn.BatchNorm1d(oup), h_swish()
        )
