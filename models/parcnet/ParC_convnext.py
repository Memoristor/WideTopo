...
class ParC_ConvNeXt(nn.Module):
    def __init__(
        ...
        stages_rs=[
            ...
            14,
            7,
        ],  # gcc block start indices, input resolutions of four stages
    ):
        super().__init__()
        ...
        for i, [depth, dim, ParC_insert_loc, input_size] in enumerate(
            zip(depths, dims, ParC_insert_locs, stages_rs)
        ):
            blocks = []
...
class ParCConvNextXT(ParC_ConvNeXt):
    """
    Constructs a mixed model, where 7*7 depth wise conv operations in the last 1/3 blocks of the last two stages are
    replaced with position aware global circular conv (GCC) operations.
    Args:
    ...
    def __init__(self, pretrained=False, in_22k=False, **kwargs):
        super().__init__(
            depths=[3, 3, 9, 3],
            dims=[48, 96, 192, 384],
            ParC_insert_locs=[3, 3, 6, 2],
