# coding=utf-8
# Original repo: https://github.com/xyongLu/SBCFormer
...
class InvertResidualBlock(nn.Module):
    def __init__(
        ...
        self.pwconv1_bn = Conv2d_BN(
            self.in_features, self.hidden_features, kernel_size=1, stride=1, padding=0
        )
        self.dwconv_bn = Conv2d_BN(
        ...
        self.pwconv2_bn = Conv2d_BN(
            self.hidden_features, self.in_features, kernel_size=1, stride=1, padding=0
        )
...
class ModifiedTransformer(nn.Module):
    def __init__(
        ...
        self.mlp = Mlp(
            ...
            hidden_features=self.dim * mlp_ratio,
...
class SBCFormerBlock(nn.Module):  # building block
    def __init__(
        ...
        for k in range(self.depth_invres):
            self.invres_blocks.add_module(
                "InvRes_{0}".format(k),
                InvertResidualBlock(
                    in_features=dim,
                    hidden_features=int(dim * invres_ratio),
                    out_features=dim,
                    kernel_size=3,
                    drop_path=0.0,
                ),
        ...
        if self.pool_ratio > 1:
            ...
            self.convTrans = nn.ConvTranspose2d(
                dim, dim, kernel_size=pool_ratio, stride=pool_ratio, groups=dim
            )
...
class SBCFormer(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size, in_chans=in_chans, embed_dim=self.embed_dims[0]
        )
        ...
        self.dpr = [
            x.item() for x in torch.linspace(0, drop_path_rate, sum(self.depths_mattn))
        ]  # stochastic depth decay rule
        cur = 0
        for i in range(self.num_stages):
            self.sbcformer_blk = SBCFormerBlock(
...
class SBCFormerXS(SBCFormer):
    def __init__(
        ...
        **kwargs,
        ...
        super(SBCFormerXS, self).__init__(
            img_size=img_size,
            ...
            depths_invres=depths_invres,
            depths_mattn=depths_mattn,
            depths_mixer=depths_mixer,
            **kwargs,
        )
...
class SBCFormerS(SBCFormer):
    def __init__(
        ...
        **kwargs,
        ...
        super(SBCFormerS, self).__init__(
            img_size=img_size,
            embed_dims=embed_dims,
            key_dim=key_dim,
            num_heads=num_heads,
...
class SBCFormerB(SBCFormer):
    def __init__(
        ...
        **kwargs,
        ...
        super(SBCFormerB, self).__init__(
            img_size=img_size,
            embed_dims=embed_dims,
            key_dim=key_dim,
            num_heads=num_heads,
...
class SBCFormerL(SBCFormer):
    def __init__(
        ...
        **kwargs,
        ...
        super(SBCFormerL, self).__init__(
            img_size=img_size,
            embed_dims=embed_dims,
            key_dim=key_dim,
            num_heads=num_heads,
