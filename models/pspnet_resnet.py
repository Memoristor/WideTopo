    ...
    def forward(self, x):
        ...
        if not self.input_shape or self.input_shape != x_shape:
            ...
            grid_w, grid_h = np.meshgrid(
                np.linspace(-1, 1, x_shape[3]), np.linspace(-1, 1, x_shape[2])
            )  # (h, w)
...
class PSPNetResNet50(PSPNet):
    def __init__(self, num_class):
        super(PSPNetResNet50, self).__init__(
            num_class=num_class, pretrained_model=models.resnet50(pretrained=True)
        )
...
class PSPNetResNet101(PSPNet):
    def __init__(self, num_class):
        super(PSPNetResNet101, self).__init__(
            num_class=num_class, pretrained_model=models.resnet101(pretrained=True)
        )
