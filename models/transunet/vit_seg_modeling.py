...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=(224, 224), num_classes=21843, zero_head=False, vis=False):
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            if posemb.size() == posemb_new.size():
                ...
                gs_new = (
                    self.img_size[0] // self.config.patches.size[0],
                    self.img_size[1] // self.config.patches.size[1],
                )
                ngs_new = gs_new[0] * gs_new[1]
                assert ngs_new != posemb_new.size()
                print(
                ...
                gs_old = _pair(int(np.sqrt(len(posemb_grid))))
                print("load_pretrained: grid-size from %s to %s" % (gs_old, gs_new))
                posemb_grid = posemb_grid.reshape(gs_old[0], gs_old[1], -1)
                zoom = (gs_new[0] / gs_old[0], gs_new[1] / gs_old[1], 1)
                posemb_grid = ndimage.zoom(posemb_grid, zoom, order=1)  # th2np
                posemb_grid = posemb_grid.reshape(1, ngs_new, -1)
                posemb = posemb_grid
