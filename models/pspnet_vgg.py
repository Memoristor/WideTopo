...
class PSPModule(nn.Module):
    def __init__(self, in_channels, out_channels=1024, pool_factors=(1, 2, 3, 6), batch_norm=True):
        ...
        bottleneck = []
        bottleneck += [
            nn.Conv2d(in_channels * (len(pool_factors) + 1), out_channels, kernel_size=1)
        ]
        if batch_norm:
...
class PSPNet(torch.nn.Module):
    def __init__(self, num_class, pretrained_model, batch_norm=True, psp_out_feature=1024):
        ...
        self.upsampling2 = PSPUpsampling(
            h_psp_out_feature, q_psp_out_feature, batch_norm=batch_norm
        )
        self.upsampling3 = PSPUpsampling(
            q_psp_out_feature, e_psp_out_feature, batch_norm=batch_norm
        )
...
class PSPNetVGG16(PSPNet):
    def __init__(self, num_class):
        super(PSPNetVGG16, self).__init__(
            num_class, pretrained_model=vgg16(pretrained=True), batch_norm=True
        )


class PSPNetVGG16BN(PSPNet):
    def __init__(self, num_class):
        super(PSPNetVGG16BN, self).__init__(
            num_class, pretrained_model=vgg16_bn(pretrained=True), batch_norm=True
        )


class PSPNetVGG19(PSPNet):
    def __init__(self, num_class):
        super(PSPNetVGG19, self).__init__(
            num_class, pretrained_model=vgg19(pretrained=True), batch_norm=True
        )


class PSPNetVGG19BN(PSPNet):
    def __init__(self, num_class):
        super(PSPNetVGG19BN, self).__init__(
            num_class, pretrained_model=vgg19_bn(pretrained=True), batch_norm=True
        )
