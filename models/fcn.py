...
__all__ = ["FCN8s", "FCN16s", "FCN32s", "FCNs"]
...
class VGGNet(VGG):
    def __init__(
        ...
        if pretrained:
            pretrained_net = getattr(models, model)(weights=VGG16_Weights.IMAGENET1K_V1)
...
class FCN32s(nn.Module):
    ...
    def forward(self, x):
        ...
        score = self.classifier(score)  # size=(N, num_class, x.H/1, x.W/1)

        # return score  # size=(N, num_class, x.H/1, x.W/1)
        return score
