        ...
        for i in range(num_elem):
            ...
            if lam != 1.0:
                if use_cutmix[i]:
                    ...
                    mixed = mixed.astype(np.float32) * lam + batch[j][0].astype(np.float32) * (
                        1 - lam
                    )
        ...
        for i in range(batch_size // 2):
            ...
            if lam < 1.0:
                if use_cutmix[i]:
                    ...
                    mixed_temp = mixed_i.astype(np.float32) * lam + mixed_j.astype(np.float32) * (
                        1 - lam
                    )
                    mixed_j = mixed_j.astype(np.float32) * lam + mixed_i.astype(np.float32) * (
                        1 - lam
                    )
                    mixed_i = mixed_temp
